...
@dp.message_handler(Command('start'))async def send_welcome(message: Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[        [InlineKeyboardButton(text="Запустить приложение", url=f"https://t.me/{bot.username}")]
    ])    await message.answer("Добро пожаловать в Clicker Bot! Нажмите 'Запустить приложение', чтобы начать.", reply_markup=keyboard)
    logger.info(f"User {message.from_user.id} started the bot.")
async def update_balance(request):    data = await request.json()
    user_id = request.headers.get('X-User-Id')    new_balance = data['balance']
    async with aiosqlite.connect('bot_database.db') as db:
        await db.execute('INSERT OR REPLACE INTO users (user_id, balance) VALUES (?, ?)', (user_id, new_balance))        await db.commit()
